// Markup.h: interface for the CMarkupST class.
//
// Markup Release 9.0
...
public:
	...
	CMarkupST() { SetDoc( NULL ); InitDocFlags(); };
	CMarkupST( MCD_CSTR szDoc ) { SetDoc( szDoc ); InitDocFlags(); };
	CMarkupST( int nFlags ) { SetDoc( NULL ); m_nFlags = nFlags; };
	CMarkupST( const CMarkupST& markup ) { *this = markup; };
	void operator=( const CMarkupST& markup );
	~CMarkupST() {};
