    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 8:
				...
				//else if( _tcscmp(pstrClass, DUI_CTR_PASSWORD) == 0 )		  pControl = new CPasswordUI;
                break;
            case 9:
				...
				//else if( _tcscmp(pstrClass, DUI_CTR_GIFBUTTON) == 0 )         pControl = new CGifButtonUI;
			...
			case 15:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_SLIFERTABLAYOUT) == 0 )   pControl = new CSliderTabLayoutUI;
                break;
            case 16:
