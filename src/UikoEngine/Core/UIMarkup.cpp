...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
    ...
    if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
		...
		{
			USES_CONVERSION;
			int nIndex= OpenZipLib(T2A((LPTSTR)sFile.GetData()),T2A((LPTSTR)sFilePwd.GetData()));
			if ( nIndex == 0 ) return _Failed(_T("OpenZip is fail!"));
			DWORD dwSize = GetZipFileSize(nIndex,(TCHAR*)pstrFilename);
			if( dwSize == 0 ) return _Failed(_T("File is empty"));
			if ( dwSize > 4096*1024 ) return _Failed(_T("File too large"));
			BYTE* pByte = new BYTE[ dwSize ];
			int res = ReadZipFile(nIndex,(TCHAR*)pstrFilename,(TCHAR*)sFilePwd.GetData(),pByte,dwSize);
			bool ret = LoadFromMem(pByte, dwSize, encoding);
			delete[] pByte;
			CloseZipLib(nIndex,false);
			return ret;
		}
