...
bool CWebBrowserUI::DoCreateControl()
{
	...
	//
	//
	//		在delaycreate属性时，调用  CActiveXUI::SetDelayCreate() 调用子类的DoCreateControl方法
	if ( m_pWebBrowser2 )
	{
		CComPtr<IOleObject> wbOleObj = NULL;
		m_pWebBrowser2->QueryInterface( IID_IOleObject ,(void**)&wbOleObj );
		if( wbOleObj )
			wbOleObj->SetClientSite(this);
	}
	//
	//
	...
	case DISPID_STATUSTEXTCHANGE:
		{
			...
			{
				//m_pWebBrowserEventHandler->StatusTextChange(pDispParams->rgvarg[0].bstrVal);
			}
		}
		break;
	case DISPID_TITLECHANGE:
		{
			if (m_pWebBrowserEventHandler != NULL)
			{
				//m_pWebBrowserEventHandler->TitleChange(pDispParams->rgvarg[0].bstrVal);
			}
		}
		break;
	case DISPID_WINDOWCLOSING:
		{
			if (m_pWebBrowserEventHandler != NULL)
			{
				//m_pWebBrowserEventHandler->WindowClosing(pDispParams->rgvarg[1].boolVal, pDispParams->rgvarg[0].pboolVal);
			}
		}
		break;
	case DISPID_PROGRESSCHANGE:
		{
			if (m_pWebBrowserEventHandler != NULL)
			{
				m_pWebBrowserEventHandler->ProgressChange(pDispParams->rgvarg[1].lVal, pDispParams->rgvarg[0].lVal);
			}
		}
		break;
	case  DISPID_DOCUMENTCOMPLETE:
		{
			if (m_pWebBrowserEventHandler != NULL)
			{
				//m_pWebBrowserEventHandler->DocumentComplete(pDispParams->rgvarg[1].pdispVal, pDispParams->rgvarg[0].pvarVal);
			}
		}
		//////////////////////////////////////////////////////////////////////////
