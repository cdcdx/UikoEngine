...
{
	//if (_tcscmp(pstrClass,_T("GifButton")) == 0) return new CGifButtonUI;
	return NULL;
}
	...
	if( pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right \
		&& pt.y >= rcCaption.top && pt.y < rcCaption.bottom ) {
            ...
            if( pControl && 
                ...
                _tcsicmp(pControl->GetClass(), _T("TreeViewUI")) != 0 && 
                _tcsicmp(pControl->GetClass(), _T("ActiveXUI")) != 0 && 
                _tcsicmp(pControl->GetClass(), _T("WebBrowserUI")) != 0 && 
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (pRoot==NULL)
	{
		MessageBox(NULL,_T("加载资源文件失败"),_T("dui"),MB_OK|MB_ICONERROR);
		ExitProcess(1);
		return 0;
	}
